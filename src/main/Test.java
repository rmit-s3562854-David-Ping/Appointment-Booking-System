...
public class Test {
	...
	public static void main(String[] args) {
		...
		LocalDateTime start = LocalDateTime.of(2017, 3, 10, 9, 0);// year,
																	// month,
																	// day,
																	// hour, min
		LocalDateTime start2 = LocalDateTime.of(2017, 3, 13, 9, 0);// year,
																	// month,
																	// day,
																	// hour, min
		LocalDateTime start3 = LocalDateTime.of(2017, 3, 7, 9, 0);// year,
																	// month,
																	// day,
																	// hour, min
		LocalDateTime start4 = LocalDateTime.of(2017, 3, 8, 9, 0);// year,
																	// month,
																	// day,
																	// hour, min
		LocalDateTime end = LocalDateTime.of(2017, 3, 10, 19, 0);// year, month,
																	// day,
																	// hour, min
		LocalDateTime end2 = LocalDateTime.of(2017, 3, 13, 19, 0);// year,
																	// month,
																	// day,
																	// hour, min
		LocalDateTime end3 = LocalDateTime.of(2017, 3, 7, 19, 0);// year, month,
																	// day,
																	// hour, min
		LocalDateTime end4 = LocalDateTime.of(2017, 3, 8, 19, 0);// year, month,
																	// day,
																	// hour, min

		Employee dummy = new Employee("Bob","marly", "s123");
		Employee dummy2 = new Employee("Ben","ten", "s132");
		...
		// DateTimeFormatter.ofPattern("dd/MM/yyyy E H:mma");
		...
		if (currentTime.toLocalTime().compareTo(business.getClosingTime()) == 1) {
			// currentTime would be between closing time to 24 so add hours
			// 24-current time which will reset the clock
			currentTime = currentTime.plusHours(24 - currentTime.getHour());// Set
																			// the
																			// clock
																			// to
																			// the
		...
		Boolean open = null;// Boolean to check whether or not current day of
							// week is valid
		Boolean appointmentPrinted = null;//
		int counter = 0;

		// 7 is a temporary hard code, we were not told what range of
		// appointment dates customers can view
		while (counter < 7) {
			...
			if (open == false) {
				...
				// Use formatter from earlier to format the Date to print it out
				// as dd/mm/yyyy (DAYOFWEEK)
				System.out.println("================");
				...
				while (currentTime.toLocalTime().compareTo(business.getClosingTime()) == -1
					...
					if (!(customer.getAppointmentArray().isEmpty())) {
						// If appointments have been made check if the time is
						...
						for (int i = 0; i < customer.getAppointmentArray().size(); i++) {
							if (customer.getAppointmentArray().get(i).getDateAndTime().equals(currentTime)) {
								// Appointment exists at this time so move
								// forward 1 duration
								currentTime = currentTime.plusMinutes(appointment.getAppointmentDuration());
								i = 0;// Make i 0 again to check the new time
										// against all the arrays again
							} else if (i == customer.getAppointmentArray().size() - 1
									&& !(customer.getAppointmentArray().get(i).getDateAndTime().equals(currentTime))) {
								// Appointment does not exist at this time
								// i has reached the end of the array and the
					...
					for (int j = 0; j < owner.getEmployeeArray().size(); j++) {
						...
						for (int k = 0; k < owner.getEmployeeArray().get(j).getStartTimes().size(); k++) {
							// New Code check every employees start time
							// arraylist and if current time is after or equal
							// to start time
							// And if it is before or equal to endtime+duration
							// then the time is valid to be listed

							// If the current employee (j) starting time is the
							// same as the current time
							// OR
							// The current employee (j) starting time is before
							// the current time

							// AND

							// If the current employee (j) ending time is the
							// same as currentTime plus duration of appointment
							// OR
							// If the current employee ending time is greater
							// than currentTime plus the duration of appointment
							...
							if ((owner.getEmployeeArray().get(j).getStartTimes().get(k).compareTo(currentTime) == 0
								...
								// If it passes all these tests it means the
								// appointment is within the opening
								// times-closing times
								// Appointment is within the employee working
								// hours
								// On a valid day
								// Is not already booked
								// Therefore it can be viewed and later be
								// booked
