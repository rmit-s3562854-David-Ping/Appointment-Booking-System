...
public class Utility {
	...
	public void exit() {
		...
		try {
			...
			writer.saveAppointments(main.getAppointmentArray());
	...
	/**
	 * Tests the input and checks which of the two formats it adheres to, will return that format
	 * @author David Ping
	 * */
	...
	/**
	 * An insertion sort method that that returns the appointments over the next
	 * week in order of dates and times, used for viewing upcoming bookings
		...
		for (int i = 0; i < main.getAppointmentArray().size(); i++) {
			if (main.getAppointmentArray().get(i).getDateAndTime().isBefore(now.plusWeeks(1))
					&& (main.getAppointmentArray().get(i).getDateAndTime().isAfter(now)
							|| main.getAppointmentArray().get(i).getDateAndTime().equals(now))
					&& sortedList.isEmpty()) {
	...
	/**
	 * An insertion sort method that sorts all appointments in the past month and returns the sorted list
	 * @author David Ping
	 */
	public List<Appointment> sortPastAppointments() {
		...
		for (int i = 0; i < main.getAppointmentArray().size(); i++) {
			if ((main.getAppointmentArray().get(i).getDateAndTime().isAfter(now.minusMonths(1))
					|| main.getAppointmentArray().get(i).getDateAndTime().equals(now.minusMonths(1)))
